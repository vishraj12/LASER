...
class BasicAgent(object):
    ...
    def __init__(self, vehicle, direction, target_speed=20, opt_dict={}, map_inst=None, grp_inst=None):
        """
        ...
        self._local_planner = LocalPlanner(self._vehicle, direction, opt_dict=opt_dict, map_inst=self._map)
        # if grp_inst:
    ...
    def set_global_road_option(self, global_road_option):
        self.global_road_option = global_road_option
        self._local_planner.set_global_road_option(global_road_option)
    ...
    def get_current_waypoint(self):
        if len(self._local_planner._waypoints_queue):
            return self._local_planner._waypoints_queue[0][0]
        else:
            return None
    ...
    def lane_change(self, direction, same_lane_time=0, other_lane_time=0, lane_change_time=1):
        """
        ...
        if not self.is_lane_following():
            print("WARNING: Ignoring the lane change as the agent is not in lane follow mode")
            return
    ...
    def _generate_lane_change_path(self, waypoint, direction='left', distance_same_lane=10,
        ...
        distance_other_lane = max(distance_other_lane, 5)
        ...
        while distance < distance_same_lane:
            next_wps = self._local_planner.get_next_wp(plan[-1][0], step_distance)
            if not next_wps:
                return []
            next_wp = next_wps[0]
        ...
        while lane_changes_done < lane_changes:
            ...
            # Move forward
            next_wps = self._local_planner.get_next_wp(plan[-1][0], lane_change_distance)
            if not next_wps:
                return []
            next_wp = next_wps[0]
    ...
    def is_lane_following(self):
        return self._local_planner.is_lane_following()
