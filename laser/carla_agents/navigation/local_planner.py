...
class LocalPlanner(object):
    ...
    def __init__(self, vehicle, direction, opt_dict={}, map_inst=None):
        """
        ...
        self.global_road_option = RoadOption.STRAIGHT
        self.logger = logging.getLogger(__name__)
        self.preparing_lane_change = False
    ...
    def _compute_next_waypoints(self, k=1):
        ...
        for _ in range(k):
            ...
            if len(next_waypoints) == 0:
                ...
                self.logger.debug(road_options_list)
                if self.global_road_option in road_options_list:
                    road_option = self.global_road_option
                else:
                ...
                self.logger.debug(road_option)
                self.logger.debug(next_waypoint.next(20)[0].transform.location)


            self._waypoints_queue.append((next_waypoint, road_option))
        self.logger.debug([(pair[0].transform.location.x, pair[0].transform.location.y) for pair in self._waypoints_queue])
    ...
    def run_step(self, debug=False):
        ...
        if num_waypoint_removed > 0:
            for _ in range(num_waypoint_removed):
                if self._waypoints_queue[0][1] != RoadOption.LANEFOLLOW:
                    self.preparing_lane_change = False
                    print("unset preparing_lane_change")
    ...
    def get_previous_wp(self, wp, dis):
        return wp.previous(dis)
...
def _retrieve_options(list_waypoints, current_waypoint):
    ...
    for next_waypoint in list_waypoints:
        ...
        # TODO modify to lane width * lane number 
        next_next_waypoint = next_waypoint.next(40.0)[0]
...
def _compute_connection(current_waypoint, next_waypoint, threshold=35):
    ...
    # TODO modify to sort and classify
